...
const GetHomeComponent = require('../../domain/use_cases/Products/GetHomeComponent');
...
module.exports = {
    ...
    async updateProduct(request, h) {
        ...
        // Treatment
        try {
            return UpdateProduct(id, name, status, serviceLocator)
        } catch (err) {
            return Boom.unauthorized('Bad credentials');
        }
    ...
    async getHomeComponent(request, h) {
        const serviceLocator = request.server.app.serviceLocator;
        // Treatment
        try {
            return GetHomeComponent(serviceLocator)
        } catch (err) {
            return Boom.unauthorized('Bad credentials');
        }

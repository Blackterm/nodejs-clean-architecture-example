...
module.exports = class extends ModuleRepository {
    ...
    async postModule(name, status) {
        ...
        return connection.query(query.insertTable('Modules', '(id,name,status, product_id)', '(?,?,?,?)'), [id, name, status, product_id]).spread((response) => response);
    }
    async updateModule(id, name, status) {
        const sqlQuery = `UPDATE Modules set name =${name},status =${status} where id = ${id}`
